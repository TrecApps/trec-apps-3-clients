...
import { TcUser } from '../models/Login';
...
export class AuthService {
  ...
  private updateProfilePic()
  {
   let observe = {
     next: (response: string) => {
       ...
       this.profilePic = `${environment.update_url}profile/file/${this.currentUser?.id}.${response}`;
     },
     error: (error: Response | any) => { 
       alert((error instanceof Response) ? error.text : (error.message ? error.message : error.toString()));
       if(error?.status == 401 || error ?.status == 403) {
   ...
   this.httpClient.get(`${environment.update_url}profile/imageType/${this.currentUser?.id}`, {responseType: 'text'}).pipe(take(1)).subscribe(observe);
  }
  ...
  async refreshUser() {
    ...
    this.httpClient.get<TcUser>(`${environment.update_url}profile/Current`,{headers: this.getHttpHeaders(true, false)}).pipe(take(1)).subscribe(observe);
  }
  ...
  logout() {
    if(this.loginToken) {
      this.httpClient.get(`${environment.update_url}Auth/logout`, {headers: new HttpHeaders().append('Authorization', this.getAuthorization())});
      this.clearAuth();
    }
    this.profilePic = this.profileFallback;
  }
